...
//              "key=${widget.apiKey}");
      ...
      if (response.statusCode == 200) {
        ...
        String road = responseJson['results'][0]['address_components'][0]['short_name'];
        String locality = responseJson['results'][0]['address_components'][1]['short_name'];
        ...
        for(var i = 0; i<components.length; i++){
          ...
          if(types.indexOf("administrative_area_level_4") > -1 || types.indexOf("postal_code")  > -1) {
            zip = item["long_name"];
          }
        }
...
class RichSuggestion extends StatelessWidget {
  ...
  List<TextSpan> getStyledTexts(BuildContext context) {
    ...
    String boldText = this.autoCompleteItem.text.substring(
        this.autoCompleteItem.offset, this.autoCompleteItem.offset + this.autoCompleteItem.length);
