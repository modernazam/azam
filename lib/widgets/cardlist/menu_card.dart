...
import 'package:tashkentsupermarket/widgets/cardlist/list_card.dart';
...
class _StateMenuCard extends State<MenuCard> {
  ...
  Future<List<Product>> getProductFromCategory(
      {categoryId, minPrice, maxPrice, orderBy, order, page}) async {
  ...
  Future<List<List<Product>>> getAllListProducts(
      {minPrice, maxPrice, orderBy, order, page = 1}) async {
    List<List<Product>> products = [];
    Services _service = Services();
